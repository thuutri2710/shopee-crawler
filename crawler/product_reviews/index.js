...
const fetchReviewsLimit = async (shopId, productId, offset, limit) => {
  const res = await fetch(
    `https://shopee.vn/api/v4/item/get_ratings?filter=0&flag=1&itemid=${productId}&limit=${limit}&offset=${offset}&shopid=${shopId}&type=0`,
    {
      headers: {
        accept: "*/*",
 ...
 * @returns {Promise<Object[]>}
 */
const crawlShopReviews = async (
  shopId,
  productId,
  offset = 0,
  limit = REVIEWS_FETCH_COUNT,
  reviewList = []
) => {
  try {
